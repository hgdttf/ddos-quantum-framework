...
Features: shaded area under curve with dot pattern, high-contrast vibrant colors.
MUST disclose PCHIP in paper caption — Q1 requirement.
Ver: 6.0 | Date: 2026-07-12
"""
...
# HIGH-CONTRAST, VIBRANT colors — saturated, distinct, publication-grade
COLORS = {
...
# Light fill colors for area under curve (same hue, very transparent)
FILL_COLORS = {
    'Classical SVM': '#E74C3C',
    'Quantum SVM': '#2ECC71',
    'Classical NN': '#3498DB',
    'Quantum NN': '#9B59B6'
}
...
def generate_figure(dataset, dataset_label):
    ...
    for clf_key, clf_name in CLASSIFIERS.items():
        ...
        # Fill area under curve with dot pattern (hatch='...')
        ax.fill_between(fpr, 0, tpr,
                        color=FILL_COLORS[clf_name],
                        alpha=0.08,  # Very light fill
                        hatch='...',  # Dot pattern
                        edgecolor=FILL_COLORS[clf_name],
                        linewidth=0.5)
    ...
    filepath = f'{OUTPUT_DIR}/roc_{safe_name}_premium.png'
...
def main():
    print("=" * 70)
    print("GENERATING PREMIUM ROC CURVES — PCHIP + DOT PATTERN + VIBRANT COLORS")
    print("=" * 70)
    ...
    print('"Polynomial (PCHIP) interpolation. Shaded areas under curves use"')
    print('"dot-pattern hatching for visual distinction. Raw empirical curves"')
    print('"are available in supplementary materials."')
    print(f"{'=' * 70}")
