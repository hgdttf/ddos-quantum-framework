#!/usr/bin/env python3
"""
Generate ROC curves with COLORED DOTS under EACH corresponding curve only.
Red dots under red line, green under green, etc.
Dots are ONLY in the area between that specific curve and the diagonal.
Ver: 10.0 | Date: 2026-07-13
"""
...
def generate_figure(dataset, dataset_label):
    ...
    for clf_key, clf_name in CLASSIFIERS.items():
        ...
        curves[clf_name] = {
            'fpr': fpr, 'tpr': tpr,
            'auc': mean_auc, 'std': std_auc,
            'color': COLORS[clf_name]
        }

    # Sort curves by AUC (lowest first) so dots don't overlap wrong curves
    sorted_names = sorted(curves.keys(), key=lambda n: curves[n]['auc'])

    # Plot dots for each curve — ONLY in that curve's area
    for clf_name in sorted_names:
        data = curves[clf_name]
        fpr = data['fpr']
        tpr = data['tpr']
        color = data['color']

        # Generate dots ONLY between this curve and diagonal
        np.random.seed(hash(clf_name + dataset) % 10000)
        n_dots = 500

        # Sample x in the range where this curve exists
        x_min = np.min(fpr[fpr > 0.01])
        x_max = np.max(fpr[fpr < 0.99])
        x_dots = np.random.uniform(x_min, x_max, n_dots)
        ...
        # Sample y between diagonal (y=x) and this curve ONLY
        y_dots = np.random.uniform(x_dots, y_curve)

        # Plot dots in THIS color ONLY
    ...
    for clf_name in sorted_names:
        ...
        ax.plot(data['fpr'], data['tpr'],
                ...
                linestyle='-',
                linewidth=2.5,
                label=f"{clf_name} (AUC = {data['auc']:.4f} ± {data['std']:.4f})",
                zorder=10)

    # Random classifier
    ax.plot([0, 1], [0, 1], 'k--', linewidth=1, alpha=0.5,
            label='Random Classifier (AUC = 0.5000)', zorder=5)
    ...
    filepath = f'{OUTPUT_DIR}/roc_{safe_name}_per_curve_dots.png'
...
def main():
    print("=" * 70)
    print("GENERATING ROC CURVES — COLORED DOTS UNDER EACH CURVE ONLY")
    print("=" * 70)
